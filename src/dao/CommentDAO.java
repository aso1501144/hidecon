...
public class CommentDAO {
	...
	public ArrayList<Comment> getcomments(String works_id) {
		...
		try { // DB接続
			...
			// INSERT文の設定・実行
			// INパラメータ(プレースホルダー)の使用例。サニタイジングのために使おう！
			String sql = "SELECT * FROM comment,user WHERE comment.user_id = user.user_id AND works_id = ?;";
			stmt = con.prepareStatement(sql);
			...
			while (rs.next()) {
				Comment cm = new Comment();
				cm.setWorks_id(rs.getString("works_id"));
				cm.setUser_name(rs.getString("user_name"));
				cm.setUser_id(rs.getString("user.user_id"));
				cm.setComment(rs.getString("comment"));
				list.add(cm);
			}
		} catch (Exception e) {
